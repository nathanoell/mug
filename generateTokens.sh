...
`./generateTokens.sh -a abc -l 3 -s 1` and it will be scattered only using file
`./generateTokens.sh -a a1b2 -l 4 -s 2` and it will be scattered using file and folders (because scatterRate > 1)
    ...
    for i in $(seq 1 $alphabetLength); do
        ...
        if [[ $todoScatterRate -gt 1 ]]; then
            ...
            if [[ $todoLength -gt 1 ]]; then # we need to call again the function
                generateToken "$(($todoLength - 1))" "0" $nextString $filePath
